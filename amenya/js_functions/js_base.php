<?php
	...
	function js_check_admin() {
		$database = new Js_Dbconn();
		$check_column = 'studentid';
		$check_for = array( 'student_group' => 'super-admin' );
		$exists = $database->exists( 'js_student', $check_column,  $check_for );
	...
	function js_new_option($title, $content, $studentid) {
		$database = new Js_Dbconn();			
	...
	function js_set_option($option, $value, $studentid) {
		$database = new Js_Dbconn();	
	...
	function js_new_admin(){
	      if ( isset( $_POST['SetAdministrator'] ) ) {			
			...
			$New_User_Details = array(		
    				...
    				'student_password' => md5(trim($_POST['password'])),
    				'student_email' => trim($_POST['email']),
    				'student_group' => 'super-admin',
    				'student_avatar' => 'student_default.jpg',
	...
	function js_homepage(){
		...
		//setcookie('temp_plural', '', time()+60*60*24*365, '/', js_getUrl());
