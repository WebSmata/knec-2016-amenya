...
class Js_Dbconn {
	...
	public function __construct()
	{
        ...
        if( $this->link->connect_errno )
        {
			...
			die(JS_I_TOP.'START SETTING UP THINGS'.JS_I_TOP_A.'<form js_page="'.js_database_setup().'" method="post">
    ...
    {
        //Delete clauses require a where param, otherwise use "trunsectione"
        if( empty( $where ) )
        {
            return false;
            exit;
    ...
    public function trunsectione( $tables = array() )
    {
        if( !empty( $tables ) )
        {
            $trunsectioned = 0;
            ...
            {
                $trunsectione = "TRUNCATE TABLE `".trim($table)."`";
                $this->link->query( $trunsectione );
                if( !$this->link->error )
                {
                    $trunsectioned++;
                }
            }
            return $trunsectioned;
        }
    }
